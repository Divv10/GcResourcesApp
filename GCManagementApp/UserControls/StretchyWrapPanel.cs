...
namespace GCManagementApp.UserControls
{
    ...
    {
        public static readonly DependencyProperty DesiredItemWidthProperty = DependencyProperty.Register(nameof(DesiredItemWidth),
        ...
        public static readonly DependencyProperty DesiredItemHeightProperty = DependencyProperty.Register(nameof(DesiredItemHeight),
                typeof(double), typeof(StretchyWrapPanel), new FrameworkPropertyMetadata(double.NaN, FrameworkPropertyMetadataOptions.AffectsMeasure));
        ...
        protected override Size MeasureOverride(Size constraint)
        {
            ...
            for (int i = 0, count = children.Count; i < count; i++)
            {
                ...
                var sz = new UVSize(Orientation,
                        itemWidthSet ? itemWidth : Math.Max(child.DesiredSize.Width, DesiredItemWidth),
                        itemHeightSet ? itemHeight : Math.Max(child.DesiredSize.Height, DesiredItemHeight));
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            for (int i = 0, count = children.Count; i < count; i++)
            {
                ...
                var sz = new UVSize(Orientation, itemWidthSet ? itemWidth : Math.Max(child.DesiredSize.Width, DesiredItemWidth), itemHeightSet ? itemHeight : Math.Max(child.DesiredSize.Height, DesiredItemHeight));
        ...
        private void ArrangeLine(double v, double lineV, int start, int end, bool useItemU, double itemU)
        {
            ...
            for (var i = start; i < end; i++)
            {
                ...
                if (child != null)
                {
                    var childSize = new UVSize(Orientation, Math.Max(child.DesiredSize.Width, DesiredItemWidth), Math.Max(child.DesiredSize.Height, DesiredItemHeight));
