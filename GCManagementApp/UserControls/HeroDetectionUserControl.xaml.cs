	...
	public partial class HeroDetectionUserControl : UserControl, INotifyPropertyChanged
	{
		...
		public int HeroesCount
		{
			...
			set
			{
				...
				if (SetProperty(ref _heroesCount, value))
				{
		...
		public string HeroesCountFormatted
		{
			get
			{
				if (HeroesCount <= 8)
				{
					if (Properties.Settings.Default.Language == "Portuguese")
					{
						return $"{HeroesCount} heróis da primeira linha";
					}
					else
					{
				...
				if (lastRow == 0)
				{
					if (Properties.Settings.Default.Language == "Portuguese")
					{
						return $"{fullRows} linhas completas";
					}
					else
					{
				...
				if (Properties.Settings.Default.Language == "Portuguese")
				{
					return $"{fullRows} linhas completas e {lastRow} heróis da ultima linha";
				}
				else
				{
		...
		public string StartTestContent => IsRunning ? TestContentStart.Stop.GetDescription() : TestContentStart.Start.GetDescription();
