...
namespace GCManagementApp.Operations {
	public sealed class AdbOperations {
		...
		public static async Task Disconnect() {
			try {
				if ( instance == null ) {
					Console.WriteLine("No active instance to disconnect");
					return;
					}

				if ( device.State != DeviceState.Online ) {
					Console.WriteLine("Device is not in online state");
					return;
					}

				await client.DisconnectAsync(instance.InstanceIpAddress);

				Console.WriteLine($"Successfully disconnected from {instance.InstanceIpAddress}");
				}
			catch ( AdbException ex ) {
				Console.WriteLine($"ADB disconnection error: {ex.Message}");
				
				if ( ex.Message.Contains("no such device") ) {
					Console.WriteLine("Device was already disconnected");
					}
				}
			catch ( Exception ex ) {
				Console.WriteLine($"General disconnection error: {ex.Message}");
				}
			finally {
				// Clean up resources
				instance = null;
